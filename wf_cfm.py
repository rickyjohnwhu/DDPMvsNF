...
slurm_config = ConfigParser("config/cfm_slurm.yml")
...
"""
config = ConfigParser("config/cfm_gmm.yml")
...
cmd = f"python run_cfm.py {cmd_args}"
...
w_range = np.array([16, 32, 48, 64, 96, 128, 256, 512, 1024, 2048])
iter_range = np.array([1, 2, 3])
for w in w_range:
    for iter in iter_range:
        config.set('params.w', w)
        config.set('params.keyword', f"{w}_varyingw_{iter}")
        args = config.compile(leaves=True, as_args=True)
        cmd_args = " ".join([f"--{k} {v}" for k, v in vars(args).items() if v is not None and k not in excluded_args])
        cmd = f"python run_cfm.py {cmd_args}"
        driver.submit_job(cmd, env="newenv", modules = ['cuda/12.1.1/'], slurm_args=slurm_args, track=True)
...
for dim in dim_range:
    for iter in iter_range:
        ...
        config.set('params.aib9_status', False)
        args = config.compile(leaves=True, as_args=True)
        cmd_args = " ".join([f"--{k} {v}" for k, v in vars(args).items() if v is not None and k not in excluded_args])
        cmd = f"python run_cfm.py {cmd_args}"
        driver.submit_job(cmd, env="newenv", modules = ['cuda/12.1.1/'], slurm_args=slurm_args, track=True)
...
for modes in modes_range:
    for iter in iter_range:
        ...
        config.set('params.keyword', f"mlp_varyingmodes_{iter}")
        args = config.compile(leaves=True, as_args=True)
        cmd_args = " ".join([f"--{k} {v}" for k, v in vars(args).items() if v is not None and k not in excluded_args])
        cmd = f"python run_cfm.py {cmd_args}"
        driver.submit_job(cmd, env="newenv", modules = ['cuda/12.1.1/'], slurm_args=slurm_args, track=True)
...
"""
"""
config = ConfigParser("config/cfm_aib9.yml")
...
for iter in iter_range:
    ...
    config.set('params.time_varying', True)
