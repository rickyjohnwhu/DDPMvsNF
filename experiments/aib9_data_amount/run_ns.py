...
def run(args):
    ...
    output_path = f"output/ns/aib9_lim={args.dataset_limiter}_seed={args.seed}"
...
#SBATCH --job-name=ns_aib9_lim
#SBATCH --output=logs/ns/aib9-lim-{dataset_limiter}-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_ns.py --config config/config_ns.yml --dataset_limiter {dataset_limiter} --seed {seed} --slurm_run
"""
