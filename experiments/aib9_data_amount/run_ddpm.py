...
def run(args):
    ...
    output_path = f"output/ddpm/aib9_lim={args.dataset_limiter}_seed={args.seed}"
...
#SBATCH --job-name=ddpm_aib9_lim
#SBATCH --output=logs/ddpm/aib9-lim-{dataset_limiter}-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_ddpm.py --config config/config_ddpm.yml --dataset_limiter {dataset_limiter} --seed {seed} --slurm_run
"""

    slurm_script_filename = f"slurm_aib9_{dataset_limiter}_{seed}.sh"
...
def main():
    ...
    dataset_limiters = [10000, 20000, 30000, 40000, 50000, 60000, 70000, 80000, 90000, 100000]
    seeds = [1, 2, 3]
