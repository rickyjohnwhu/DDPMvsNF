...
def run(args):
    ...
    output_path = f"output/ddpm/aib9_seed={args.seed}"
...
def create_slurm_script(seed, config):
    script = f"""#!/bin/bash
...
#SBATCH --job-name=ddpm_aib9
#SBATCH --output=logs/ddpm/aib9-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_ddpm.py --config config/config_ddpm.yml --seed {seed} --slurm_run
"""

    slurm_script_filename = f"slurm_aib9_{seed}.sh"
...
def main():
    ...
    if args.use_slurm:
        ...
        for seed in seeds:
            ...
            slurm_script = create_slurm_script(seed, config)
