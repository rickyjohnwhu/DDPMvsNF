...
def run(args):
    ...
    output_path = f"output/cfm/tune={args.w}_seed={args.seed}"
...
def create_slurm_script(w, seed, config):
    script = f"""#!/bin/bash
...
#SBATCH --job-name=cfm_tune
#SBATCH --output=logs/cfm/tune-{w}-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_cfm.py --config config/config_cfm.yml --w {w} --seed {seed} --slurm_run
"""

    slurm_script_filename = f"slurm_{w}_{seed}.sh"
...
def main():
    ...
    ws = [16, 32, 48, 64, 96, 128, 256, 512, 1024, 2048]
    seeds = [1, 2, 3]
    ...
    if args.use_slurm:
        ...
        for w in ws:
            for seed in seeds:
                ...
                slurm_script = create_slurm_script(w, seed, config)
