...
def run(args):
    ...
    output_path = f"output/cfm/modes={args.path_modes}_seed={args.seed}"
...
#SBATCH --job-name=cfm_modes
#SBATCH --output=logs/cfm/modes-{path_modes}-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_cfm.py --config config/config_cfm.yml --input {input} --path_modes {path_modes} --seed {seed} --slurm_run
"""
