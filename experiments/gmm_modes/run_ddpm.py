...
def run(args):
    ...
    output_path = f"output/ddpm/modes={args.path_modes}_seed={args.seed}"
...
#SBATCH --job-name=ddpm_modes
#SBATCH --output=logs/ddpm/modes-{path_modes}-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_ddpm.py --config config/config_ddpm.yml --input {input} --path_modes {path_modes} --seed {seed} --slurm_run
"""
