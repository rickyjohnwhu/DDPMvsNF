...
def run(args):
    ...
    output_path = f"output/ddpm/asym={args.asym}_seed={args.seed}"
...
def create_slurm_script(input, asym, seed, config):
    script = f"""#!/bin/bash
...
#SBATCH --job-name=ddpm_asym
#SBATCH --output=logs/ddpm/asym-{asym}-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_ddpm.py --config config/config_ddpm.yml --input {input} --asym {asym} --seed {seed} --slurm_run
"""

    slurm_script_filename = f"slurm_{asym}_{seed}.sh"
...
def main():
    ...
    parser.add_argument("--asym", type=float)
    ...
    inputs = ['../../datasets/data_input/gmm_dim_50_modes_2_asym_0.001.npy', '../../datasets/data_input/gmm_dim_50_modes_2_asym_0.005.npy', '../../datasets/data_input/gmm_dim_50_modes_2_asym_0.01.npy', '../../datasets/data_input/gmm_dim_50_modes_2_asym_0.025.npy', '../../datasets/data_input/gmm_dim_50_modes_2_asym_0.05.npy', '../../datasets/data_input/gmm_dim_50_modes_2_asym_0.1.npy', '../../datasets/data_input/gmm_dim_50_modes_2_asym_0.15.npy', '../../datasets/data_input/gmm_dim_50_modes_2_asym_0.25.npy', '../../datasets/data_input/gmm_dim_50_modes_2_asym_0.35.npy', '../../datasets/data_input/gmm_dim_50_modes_2_asym_0.5.npy']
    asyms = [0.001, 0.005, 0.01, 0.025, 0.05, 0.1, 0.15, 0.25, 0.35, 0.5]
    seeds = [1, 2, 3]
    ...
    if args.use_slurm:
        ...
        for i in range(len(inputs)):
            for seed in seeds:
                ...
                slurm_script = create_slurm_script(inputs[i], asyms[i], seed, config)
