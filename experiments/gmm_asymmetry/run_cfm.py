...
def run(args):
    ...
    output_path = f"output/cfm/asym={args.asym}_seed={args.seed}"
...
#SBATCH --job-name=cfm_asym
#SBATCH --output=logs/cfm/asym-{asym}-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_cfm.py --config config/config_cfm.yml --input {input} --asym {asym} --seed {seed} --slurm_run
"""
