...
def run(args):
    ...
    output_path = f"output/ns/asym={args.asym}_seed={args.seed}"
...
#SBATCH --job-name=ns_asym
#SBATCH --output=logs/ns/asym-{asym}-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_ns.py --config config/config_ns.yml --input {input} --asym {asym} --seed {seed} --slurm_run
"""
