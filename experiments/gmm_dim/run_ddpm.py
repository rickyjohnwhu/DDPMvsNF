...
def run(args):
    ...
    output_path = f"output/ddpm/dim={args.path_dim}_seed={args.seed}"
...
#SBATCH --job-name=ddpm_dim
#SBATCH --output=logs/ddpm/dim-{path_dim}-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_ddpm.py --config config/config_ddpm.yml --path_dim {path_dim} --input {input} --seed {seed} --slurm_run
"""
