...
def run(args):
    ...
    output_path = f"output/ns/dim={args.path_dim}_seed={args.seed}"
...
#SBATCH --job-name=ns_dim
#SBATCH --output=logs/ns/dim-{path_dim}-seed-{seed}.out

source $MAMBA_ROOT_PREFIX/etc/profile.d/micromamba.sh
micromamba activate compenv

python run_ns.py --config config/config_ns.yml --path_dim {path_dim} --input {input} --seed {seed} --slurm_run
"""
